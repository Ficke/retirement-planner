...
static BEND_POINTS: Lazy<Vec<SSABendPoint>> = Lazy::new(|| {
    vec![
        SSABendPoint { threshold: Some(1174.0), rate: 0.90 },
        SSABendPoint { threshold: Some(7078.0), rate: 0.32 },
        SSABendPoint { threshold: None, rate: 0.15 },
    ]
});
...
pub fn calculate_aime(salary_history: &[f64]) -> f64 {
    ...
    let total_earnings: f64 = top_35_years.iter().sum();
    let months_of_earnings = top_35_years.len() * 12;
    
    if months_of_earnings > 0 {
        total_earnings / months_of_earnings as f64
    } else {
        0.0
    }
}
...
pub fn get_claim_age_adjustment(claim_age: u32) -> f64 {
    CLAIM_AGE_ADJUSTMENTS.get(&claim_age).copied().unwrap_or(1.0)
}
...
pub fn estimate_salary_history(
    ...
    let years_of_work = (retirement_age - CAREER_START_AGE).min(35);
    
    // Project backwards from current salary to estimate career progression
    for years_ago in (0..years_of_work).rev() {
        let salary_that_year = current_salary / (1.0 + salary_growth_rate).powi(years_ago as i32);
        salary_history.push(salary_that_year);
    }
...
mod tests {
    ...
    fn test_aime_calculation() {
        ...
        let aime = calculate_aime(&salary_history);
        let expected = 50000.0 / 12.0; // Monthly average
        assert!((aime - expected).abs() < 0.01);
    }
    ...
    fn test_pia_calculation() {
        ...
        assert!((pia - expected).abs() < 0.01);
    }
    
    #[test]
    fn test_claim_age_adjustments() {
        assert_eq!(get_claim_age_adjustment(62), 0.75); // Early claiming
    ...
    fn test_estimate_salary_history() {
        ...
        // Should have at most 35 years (65 - 22 = 43, capped at 35)
        assert!(history.len() <= 35);
